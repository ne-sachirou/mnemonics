defmodule Mnemonics.SnapTest do
  ...
  defp build_snap, do: Snap.new |> Snap.snap(:examples, 1) |> put_in([:examples, :cache, 1], "example 1")
  ...
  describe "Kernel.get_and_update_in/3" do
    ...
    test "Update cache" do
      assert {"example 1", snap} = get_and_update_in build_snap()[:examples].cache[1], fn value ->
  ...
  describe "Kernel.pop_in/2" do
    test "pop_in function" do
      assert {%{version: 1, cache: %{1 => "example 1"}}, Snap.new} == pop_in build_snap(), [:examples]
